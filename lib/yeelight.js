//base from: https://github.com/cahek2202/ioBroker.yeelight
	...
	socket.on('data', function (data) {
		if (callback) {
			try {
				...
				seperateJSON(data,function(err, result){
					if(err){
						callback(err);
						return
					}
...
function seperateJSON(string, callback){
    var jsons = string.toString().replace(/} {/g,'},{');
    ...
    try {
        ...
        callback(new Error('yeelight.js.seperateJSON: could not extract Json from multiple message_ '+ jsons));
        return
    }
    console.log(JSON.stringify(jsons[1]))
    const result = jsons.find( jsons => jsons.id === 0);

    if(result){
        callback(null,result)
    }
    else{
        callback(new Error('yeelight.js.seperateJSON:could find answer in string_ '+ JSON.stringify(jsons)));
    }
