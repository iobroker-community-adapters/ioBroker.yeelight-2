...
>>>>>>> 7bd60b4dae56c29715f715eb6c039ed75f33d285
				return;
			}
			if (data['error']) {
				callback(new Error(data['error']['message']));
...
function seperateJSON(string, callback){
    ...
    try {
        ...
        callback(new Error('could not extract Json from multiple message'));
        return
    }
    ...
    if(result){
        ...
        callback(new Error('could find answer in string:_'+ JSON.stringify(jsons)));
    }
